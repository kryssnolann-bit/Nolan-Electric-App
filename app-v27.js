...
document.addEventListener("click",async e=>{
 ...
 const inv=e.target.closest("[data-invoice-status]");if(inv){if(!state.selected||isFieldUser())return;const status=inv.dataset.invoiceStatus,id=inv.dataset.invoiceId;const patch=status==="paid"?{status,paid_at:new Date().toISOString()}:{status,paid_at:null};const r=await sb.from("invoices").update(patch).eq("id",id);if(r.error){toast(r.error.message);return}if(status==="sent")await sb.from("jobs").update({status:"invoiced"}).eq("id",state.selected.id);if(status==="paid")await sb.from("jobs").update({status:"paid"}).eq("id",state.selected.id);if(status==="sent"||status==="draft")await sb.from("jobs").update({status:status==="sent"?"invoiced":"complete"}).eq("id",state.selected.id);await load();return}
