...
function render(){normalizeTab();root.innerHTML=session?appShell():login();const oldModal=document.getElementById("job-modal-root");if(oldModal) oldModal.remove();if(session&&state.selected){const host=document.createElement("div");host.id="job-modal-root";host.innerHTML=jobModal(state.selected);document.body.appendChild(host)}}
