...
let state={tab:"dashboard",jobs:[],customers:[],tasks:[],members:[],profiles:[],timeEntries:[],employeeRates:{},jobCosts:[],invoices:[],companyTimeEntries:[],companyJobCosts:[],loading:false,selected:null};
...
function dashboard(){
 ...
 // Recent Jobs should show the newest jobs regardless of status, including completed, invoiced, and paid jobs.
 const recent=[...state.jobs].sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0)).slice(0,8);
 ...
 <h3>Recent Jobs</h3>${jobList(recent)}</div>`;
}
...
function printInvoice(inv,j){
 const c=j.customers||{};
 const addr=[j.address_line1,j.city,j.state,j.postal_code].filter(Boolean).join(", ");
 const invoiceDate=inv.created_at?dateFmt(inv.created_at):dateFmt(new Date());
 const status=inv.status||"draft";
 const html=`<!doctype html><html><head><meta charset="utf-8"><title>${esc(inv.invoice_number||"Invoice")} · Nolan Electric</title>
 <style>
 *{box-sizing:border-box}body{margin:0;font-family:Arial,Helvetica,sans-serif;color:#111;background:#fff}.page{max-width:820px;margin:0 auto;padding:48px}.top{display:flex;justify-content:space-between;gap:30px;border-bottom:3px solid #111;padding-bottom:22px}.brand{font-size:30px;font-weight:800}.muted{color:#666}.right{text-align:right}.title{font-size:30px;font-weight:800;margin:34px 0 18px}.boxes{display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:28px}.box h4{margin:0 0 8px;text-transform:uppercase;font-size:11px;color:#666;letter-spacing:.08em}.box div{font-size:15px;line-height:1.55}.table{width:100%;border-collapse:collapse;margin-top:24px}.table th{background:#f0f0f0;text-align:left;padding:12px;border-bottom:1px solid #ccc;font-size:12px}.table td{padding:16px 12px;border-bottom:1px solid #ddd}.num{text-align:right}.totals{margin-left:auto;width:320px;margin-top:18px}.totals div{display:flex;justify-content:space-between;padding:7px 0}.grand{font-size:20px;font-weight:800;border-top:2px solid #111;margin-top:8px;padding-top:12px}.status{display:inline-block;padding:6px 10px;border:1px solid #111;border-radius:999px;text-transform:uppercase;font-size:11px;font-weight:700}.footer{margin-top:60px;border-top:1px solid #ddd;padding-top:18px;color:#666;font-size:12px}.printbar{position:fixed;right:20px;top:20px}.printbar button{background:#111;color:#fff;border:0;border-radius:8px;padding:12px 16px;font-size:14px;font-weight:700}@media print{.printbar{display:none}.page{padding:0;max-width:none}}
 </style></head><body><div class="printbar"><button onclick="window.print()">Print / Save PDF</button></div>
 <main class="page"><div class="top"><div><div class="brand">Nolan Electric</div><div class="muted">Electrical Contractor</div></div><div class="right"><div class="title" style="margin:0 0 8px">INVOICE</div><div><b>${esc(inv.invoice_number||"Invoice")}</b></div><div class="muted">Invoice Date: ${invoiceDate}</div><div class="muted">Due Date: ${dateFmt(inv.due_date)}</div><div style="margin-top:8px"><span class="status">${esc(status)}</span></div></div></div>
 <div class="boxes"><div class="box"><h4>Bill To</h4><div><b>${esc(c.name||"Customer")}</b></div>${c.company?`<div>${esc(c.company)}</div>`:""}${c.phone?`<div>${esc(c.phone)}</div>`:""}${c.email?`<div>${esc(c.email)}</div>`:""}</div><div class="box"><h4>Job</h4><div><b>${esc(j.name||"Electrical Services")}</b></div><div>${esc(j.job_number||"")}</div>${addr?`<div>${esc(addr)}</div>`:""}</div></div>
 <table class="table"><thead><tr><th>Description</th><th class="num">Qty</th><th class="num">Amount</th></tr></thead><tbody><tr><td><b>Electrical services</b><br><span class="muted">${esc(j.description||"Electrical work performed for this job.")}</span></td><td class="num">1</td><td class="num">${money(inv.subtotal)}</td></tr></tbody></table>
 <div class="totals"><div><span>Subtotal</span><span>${money(inv.subtotal)}</span></div><div><span>Tax</span><span>${money(inv.tax)}</span></div><div class="grand"><span>Total</span><span>${money(inv.total)}</span></div>${status==="paid"?`<div><span>Paid</span><span>${money(inv.total)}</span></div><div><b>Balance Due</b><b>$0.00</b></div>`:`<div><b>Balance Due</b><b>${money(inv.total)}</b></div>`}</div>
 <div class="footer"><b>Thank you for your business.</b><br>Nolan Electric · Invoice ${esc(inv.invoice_number||"")} · Job ${esc(j.job_number||"")}</div></main></body></html>`;
 const w=window.open("","_blank","width=900,height=900");
 if(!w){toast("Your browser blocked the invoice window. Allow pop-ups for this site and try again.");return}
 w.document.open();w.document.write(html);w.document.close();w.focus();
}

function jobPage(j){return `<div class="job-page"><div class="job-page-inner"><div class="job-page-back"><button class="btn" id="close">← Back to ${esc(j.name)}</button></div>${jobModal(j)}</div></div>`}
...
function jobModal(j){
 ...
 const invoicePanel=fieldUser?"":`<div class="card" style="margin:12px 0"><div class="row"><div><h3 style="margin:0">Invoicing</h3><div class="sub">${state.invoices.length?`${state.invoices.length} invoice${state.invoices.length===1?"":"s"}`:"No invoices yet"}</div></div><button class="btn primary" id="createInvoice">+ Invoice</button></div><div class="list" style="margin-top:10px">${state.invoices.map(inv=>`<div class="job"><div class="row"><div><b>${esc(inv.invoice_number||"Invoice")}</b><div class="sub">${statusLabel(inv.status)} · Due ${dateFmt(inv.due_date)}</div></div><b>${money(inv.total)}</b></div><div class="field-actions" style="margin-top:8px">${inv.status==="draft"?`<button class="btn" data-invoice-status="sent" data-invoice-id="${inv.id}">Mark Sent</button>`:inv.status==="sent"?`<button class="btn primary" data-invoice-status="paid" data-invoice-id="${inv.id}">Mark Paid</button>`:`<button class="btn" data-invoice-status="sent" data-invoice-id="${inv.id}">Mark Unpaid</button>`}</div><button class="btn" data-print-invoice="${inv.id}">🖨️ Print / Save PDF</button></div></div>`).join("")||"<div class='empty'>No invoices yet.</div>"}</div><div class="row" style="margin-top:10px"><div class="sub">Invoiced: ${money(state.invoices.reduce((a,x)=>a+Number(x.total||0),0))} · Paid: ${money(state.invoices.filter(x=>x.status==="paid").reduce((a,x)=>a+Number(x.total||0),0))} · Outstanding: ${money(state.invoices.filter(x=>x.status!=="paid").reduce((a,x)=>a+Number(x.total||0),0))}</div></div></div>`;
 ...
 return `<div class="job-sheet"><div class="row"><div><h2>${esc(j.job_number)} · ${esc(j.name)}</h2><div class="sub">${esc(j.customers?.name||"No customer")} · ${statusLabel(j.status)}</div></div><button class="btn" id="close">Close</button></div><hr>
 ${financial}${schedulePanel}${invoicePanel}
 ${timePanel}
...
async function load(){
 ...
 try{
  ...
  const jr=await sb.from("jobs").select("*,customers(name,company,phone,email)").order("created_at",{ascending:false});
  if(jr.error)throw new Error("Jobs could not be loaded: "+jr.error.message);
  const allJobs=jr.data||[];
  // Keep the open job detail synchronized with the freshly loaded database row.
  if(state.selected){
   const fresh=allJobs.find(x=>x.id===state.selected.id);
  ...
  if(state.selected){
   ...
   if(!isFieldUser()){
    const iv=await sb.from("invoices").select("*").eq("job_id",state.selected.id).order("created_at",{ascending:false});
    state.invoices=iv.error?[]:(iv.data||[]);
  ...
  }else{state.tasks=[];state.members=[];state.timeEntries=[];state.jobCosts=[];state.invoices=[];}
...
document.addEventListener("click",async e=>{
 ...
 if(e.target.id==="createInvoice"){if(!state.selected||isFieldUser())return;const j=state.selected;const allInv=await sb.from("invoices").select("invoice_number").order("created_at",{ascending:false}).limit(200);let maxNum=0;(allInv.data||[]).forEach(x=>{const m=String(x.invoice_number||"").match(/^INV-\d{4}-(\d+)$/);if(m)maxNum=Math.max(maxNum,Number(m[1]));});const next="INV-"+String(new Date().getFullYear())+"-"+String(maxNum+1).padStart(3,"0");const suggested=Number(j.contract_amount||0);modalForm("Create Invoice",`<form class="form"><input name="invoice_number" value="${next}" placeholder="Invoice #" required><input name="subtotal" type="number" min="0" step=".01" value="${suggested}" placeholder="Subtotal" required><input name="tax" type="number" min="0" step=".01" value="0" placeholder="Tax"><input name="due_date" type="date" value="${new Date(Date.now()+30*86400000).toISOString().slice(0,10)}"><p class="sub">Total is calculated from subtotal + tax. This invoice is admin-only.</p><button class="btn primary" type="submit">Create Invoice</button></form>`,async f=>{const subtotal=Number(f.get("subtotal")||0),tax=Number(f.get("tax")||0);return sb.from("invoices").insert({job_id:j.id,invoice_number:f.get("invoice_number"),status:"draft",subtotal,tax,total:subtotal+tax,due_date:f.get("due_date")||null})});return}
 const printBtn=e.target.closest("[data-print-invoice]");if(printBtn){if(!state.selected||isFieldUser())return;const inv=state.invoices.find(x=>x.id===printBtn.dataset.printInvoice);if(inv)printInvoice(inv,state.selected);return}
 const inv=e.target.closest("[data-invoice-status]");if(inv){
  if(!state.selected||isFieldUser())return;
  const status=inv.dataset.invoiceStatus,id=inv.dataset.invoiceId;
  const patch=status==="paid"?{status,paid_at:new Date().toISOString()}:{status,paid_at:null};
  const r=await sb.from("invoices").update(patch).eq("id",id);
  if(r.error){toast(r.error.message);return}
  const nextJobStatus=status==="paid"?"paid":status==="sent"?"invoiced":"complete";
  const jr=await sb.from("jobs").update({status:nextJobStatus}).eq("id",state.selected.id);
  if(jr.error){toast("Invoice updated, but job status could not be updated: "+jr.error.message);return}
  await load();
  return;
 }
